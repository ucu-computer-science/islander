 ...
 * current MOUNT and PID isle. */
static void configure_procfs() {
    // Create "/proc" directory with certain permissions.
    if (mkdir("/proc", READ_N_EXEC_PERMISSIONS) && errno != EEXIST)
        kill_process("Failed to mkdir /proc: %m\n");

    // Mount "/proc" in current MNT and PID isle.
    if (mount("proc", "/proc", "proc", 0, ""))
        kill_process("Failed to mount proc: %m\n");
}
...
static void setup_mntns(char *rootfs) {
    ...
    if (mount(rootfs, mnt, EXT4, MS_BIND, ""))
        kill_process("Failed to mount %s at %s: %m\n", rootfs, mnt);

    // Change the current directory to "rootfs".
    if (chdir(mnt))
        kill_process("Failed to chdir to rootfs mounted at %s: %m\n", mnt);

    // Create folder 'put_old' for the old root file system.
    const char *put_old = ".put_old";
    if (mkdir(put_old, ALL_PERMISSIONS) && errno != EEXIST)
        kill_process("Failed to mkdir put_old %s: %m\n", put_old);
    ...
    if (syscall(SYS_pivot_root, ".", put_old))
        kill_process("Failed to pivot_root from %s to %s: %m\n", rootfs, put_old);

    // Change the current directory to "/" -> Root.
    if (chdir("/"))
        kill_process("Failed to chdir to new root: %m\n");
    ...
    if (umount2(put_old, MNT_DETACH))
        kill_process("Failed to umount put_old %s: %m\n", put_old);
}
